...
export default function Leaderboard() {
  ...
  const { leaderboard, isLoading } = useLeaderboard(period);
  const userRank = user ? leaderboard.find(entry => entry.userId === user.id) : null;
          ...
          <TabsContent value={period}>
            {isLoading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground">
                  {language === 'pl'
                    ? 'Brak danych w rankingu. Zacznij się uczyć!'
                    : 'No leaderboard data yet. Start learning!'}
                </p>
              </div>
            ) : (
              <>
                ...
                {/* Current user position if not in leaderboard */}
